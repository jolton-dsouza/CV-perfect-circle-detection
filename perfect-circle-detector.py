...
#Uses skimage module
def snakes(img,img_gray,circles):
    
    largest_contour = list()
    
    for i in circles[0,:]:
        print("Circle centre x-cord is %d , Circle centre y-cord is %d and the radius is %d" %( i[0], i[1], i[2]))
        #draw the center
        cv2.circle(img,(int(i[0]),int(i[1])),3,(0,0,255),-1,8,0)
        # draw the outer circle
        cv2.circle(img,(int(i[0]),int(i[1])),int(i[2]*1.7),(255,0,0),2,8,0)
...
def main():
    frames = list()
    frame_no = 0; max_val = 0; max_val_frame_no = 0
    #capture = cv2.VideoCapture(sys.argv[1])

    capture = cv2.VideoCapture('C:/Users/jolto/Desktop/Interviews/KLA/KLAPresentation/sample.mov')
    t0 = time.time()
    while (capture.isOpened()):
        ...
        if(type(src) == type(None)):
            ...
            circle_contour = snakes(src,src_gray,circles)
